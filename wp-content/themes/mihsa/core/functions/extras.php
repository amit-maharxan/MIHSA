...
function mihsa_upload_mime_types($mimes) {
  $mimes['svg'] = 'image/svg+xml';
  return $mimes;
}
add_filter('upload_mimes', 'mihsa_upload_mime_types');


// Resize image using img thumb
if ( !( function_exists('mihsa_imgsize') ) ) :
  // Use of phpthumb -- Resize your image appearing on a site
  function mihsa_imgsize($w, $h, $src, $fr=1) {
    if ($w != 0 && $h == 0):
